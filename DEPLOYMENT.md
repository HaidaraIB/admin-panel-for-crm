# دليل نشر CRM-admin-panel على VPS للإنتاج

هذا الدليل يشرح كيفية نشر مشروع CRM-admin-panel على خادم VPS في بيئة الإنتاج.
...
```bash
cd /var/www
sudo git clone <repository-url> crm-admin-panel
sudo chown -R $USER:$USER /var/www/crm-admin-panel
cd crm-admin-panel
```
...
```bash
# من جهازك المحلي
scp -r CRM-admin-panel/* user@your-server-ip:/var/www/crm-admin-panel/
```
...
```bash
sudo nano /etc/nginx/sites-available/crm-admin-panel
```
...
server {
    listen 80;
    server_name admin.loop-crm.app www.admin.loop-crm.app;

    root /var/www/crm-admin-panel/dist;
    index index.html;
...
#### 5.2 تفعيل الموقع
```bash
sudo ln -s /etc/nginx/sites-available/crm-admin-panel /etc/nginx/sites-enabled/
...
```bash
sudo certbot --nginx -d admin.loop-crm.app -d www.admin.loop-crm.app
```
...
```bash
nano /var/www/crm-admin-panel/ecosystem.config.js
```
...
```bash
sudo chown -R www-data:www-data /var/www/crm-admin-panel/dist
sudo chmod -R 755 /var/www/crm-admin-panel
```
...
```bash
nano /var/www/crm-admin-panel/update.sh
```
...
```bash
chmod +x /var/www/crm-admin-panel/update.sh
```
...
- تحقق من أذونات الملفات: `ls -la /var/www/crm-admin-panel/dist`
- تأكد من أن Nginx يقرأ من المجلد الصحيح

### المشكلة: خطأ في الاتصال بالـ API
- تحقق من أن `VITE_API_URL` يشير إلى عنوان API الصحيح
- تأكد من أن API يعمل ويمكن الوصول إليه
- تحقق من إعدادات CORS في API للسماح بالنطاق الجديد
...
5. **إعدادات API**: تأكد من أن API يدعم النطاق الجديد في إعدادات CORS

## هيكل الملفات النهائي

```
/var/www/crm-admin-panel/
...
- إعدادات CORS في API
- اتصال الشبكة بين Admin Panel و API
